...
基线 sqlite 开箱；miss 则 `index --repo --source <用户确认的 clone>`。禁止为查行号 clone，也禁止自行猜测源码路径（例如某盘 `Code\msprof`）。用户给出路径后写入 `~/.agent-skills/config.json` 的 `cann.repo_paths` 或 `catalogs/repos.local.json`，以后按配置用。所有级别都能对。6 位错误码带 title/仓（`cann_error_codes.json`）。
